...
declare global {
  interface Window {
    __clipsageSupabaseClient?: SupabaseClient;
  }

  var __clipsageSupabaseClient: SupabaseClient | undefined;
}

function createClipSageSupabaseClient() {
  return createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
    auth: {
      storageKey: "clipsage-auth",
      persistSession: true,
...
function getSupabaseSingleton() {
  if (typeof window !== "undefined") {
    if (!window.__clipsageSupabaseClient) {
      window.__clipsageSupabaseClient = createClipSageSupabaseClient();
    }

    return window.__clipsageSupabaseClient;
  }

  if (!globalThis.__clipsageSupabaseClient) {
    globalThis.__clipsageSupabaseClient = createClipSageSupabaseClient();
  }

  return globalThis.__clipsageSupabaseClient;
}
...
async function getSafeSession() {
  if (!sharedSessionPromise) {
    sharedSessionPromise = supabase.auth
      .getSession()
...
const SEARCH_URL =
  "https://sstcthkqleegmvessfxa.supabase.co/functions/v1/search-clips";
    ...
    <>
      {parts.map((part, index) => {
        ...
        return (
          <mark
            key={index}
            className="rounded-md bg-yellow-400 px-1 font-black text-black"
          >
            {part}
          </mark>
        );
...
export default function ClipSageApp() {
  ...
  const premiumRefreshId = useRef(0);
  ...
  async function checkTableForPremium(
    table: string,
    column: string,
    value: string
  ) {
  ...
  async function refreshPremiumStatus(currentSession?: Session | null) {
    const refreshId = premiumRefreshId.current + 1;
    premiumRefreshId.current = refreshId;
    setPremiumChecked(false);

    const workingSession = currentSession ?? session;

    if (!workingSession?.user?.id) {
      if (premiumRefreshId.current === refreshId) {
        setIsPremium(false);
        setPlan("free");
    ...
    try {
      const checks = [
        ...
        userEmail
          ? checkTableForPremium("profiles", "email", userEmail)
          : Promise.resolve(false),
        checkTableForPremium("customers", "user_id", userId),
        userEmail
          ? checkTableForPremium("customers", "email", userEmail)
          : Promise.resolve(false),
        checkTableForPremium("subscriptions", "user_id", userId),
        userEmail
          ? checkTableForPremium("subscriptions", "email", userEmail)
          : Promise.resolve(false),
      ];
  ...
  useEffect(() => {
    ...
    const {
      ...
      setSession(currentSession);
      void refreshPremiumStatus(currentSession);
    });
    ...
    try {
      ...
      const finalMessage =
        err instanceof Error
          ? err.message
          : "Something went wrong opening billing.";
  ...
  async function fetchClips(searchQuery: string, pageToFetch: number) {
    ...
    if (!response.ok) {
      if (data?.error === "limit_reached") {
        setSearchesLeft(0);
        throw new Error(
          data?.message || "You’ve reached your 10 free searches for today."
        );
      }
  ...
  async function showMoreClips() {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Something went wrong while loading more clips."
      );
    } finally {
      setLoadingMore(false);
    }
  ...
  const displayedSearchesLeft =
    searchesLeft === null ? searchLimit ?? 10 : searchesLeft;
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mx-auto mb-16 max-w-5xl text-center">
          ...
          <p className="mx-auto mb-10 max-w-3xl text-lg leading-8 text-blue-100">
            Search podcasts, interviews, and long-form videos to pull up the
            exact part you need in seconds. Built for podcast lovers, creators,
            and researchers!
          </p>
            ...
            <div className="mt-5 grid gap-5 text-left md:grid-cols-[1fr_auto] md:items-center">
              <div>
                ...
                <p className="mt-4 text-lg font-black text-white">
                  {isFree
                    ? `${displayedSearchesLeft} free searches left`
            ...
            <section key={animationKey} className="grid gap-7 md:grid-cols-3">
              {results.map((clip, index) => {
                ...
                return (
                  <article
                    key={`${clip.id || "clip"}-${
                      clip.youtube_video_id || "video"
                    }-${clip.start_seconds || 0}-${index}`}
